...
**A build does not get your `.env`.** `EXPO_PUBLIC_*` values are inlined when the bundle is built, `.env` is gitignored, and EAS Build uploads the project the way git sees it, so the file never arrives. The result is not a crash, which is what makes it expensive: `isSupabaseConfigured` goes false, `authStatus` resolves to `'disabled'`, the route gate treats that as signed in, and a store build installs as the local-only app with no sign in screen at all. It shipped once. The credentials now live as EAS environment variables (`eas env:list production`), each profile in `eas.json` names its environment, and `app.config.js` throws on an EAS build that is missing either one. `GARAGE_ALLOW_NO_BACKEND=1` is the deliberate opt out. Keep `app.config.js` a guard only: everything static stays in `app.json`.
